...
chccs_suspensions_num <- 255
chccs_demographics %>% 
  filter(year == 2020) %>% 
  cbind(., chccs_suspensions %>% 
          filter(year == 2020) %>% 
          pivot_wider(!pct, names_from = subgroup, values_from = num) %>% 
          summarize(sus_white = (white_female + white_male) / chccs_suspensions_num * 100, 
                    sus_black = (black_female + black_male) / chccs_suspensions_num * 100,
                    sus_hispanic = (his_female + his_male) / chccs_suspensions_num * 100, 
                    sus_asian = (asian_female + asian_male) / chccs_suspensions_num * 100,
                    sus_multi = (multi_female + multi_male) / chccs_suspensions_num * 100)) %>% 
